...
- **Time-entry creation posts to top-level `POST /TimeEntries`** ([#72](https://github.com/GDSTechnology/autotask-mcp/issues/72)). Ticket/task time entries were created via child routes (`/Tickets/{id}/TimeEntries`, `/Tasks/{id}/TimeEntries`) that Autotask returns **404** for (verified live) — so scoped time-entry creation was broken. It now posts to `/TimeEntries` with the parent id in the body. `TimeEntries` has **no `projectID`** field (project work logs against a project *task*), so `projectID` is dropped and a project-only entry is rejected with a clear message. Unblocks programmatic time-entry backfill (#42).
- **Block-hour usage now consumes `hoursToBill`, not raw `hoursWorked`** ([#43](https://github.com/GDSTechnology/autotask-mcp/issues/43)). Techs enter start/stop or total time; Autotask's billing rules (15-minute rounding) produce `hoursToBill`, which is what is actually drawn against the monthly block. `autotask_report_block_hour_usage` now draws each billable entry down by `hoursToBill` (falling back to `hoursWorked` when it isn't set), so block consumption/overage/forfeiture match what Autotask bills.
- **Read path fails closed on an unusable Autotask response.** `AutotaskHttpClient.request()` previously turned three anomalies on an HTTP 2xx into a silent empty result: a body-read that aborted mid-stream (the 60s timeout firing after headers) became `''` via `.catch(() => '')`, an empty body returned `undefined`, and a body that failed `JSON.parse` (truncation) returned `undefined`. `get()` then reported that as `null` (a false "not found") and `query()` as `[]` (a false "zero records") — which during a large project build session masqueraded as reads with no payload, phase/task lookups returning zero, and a just-created record reading back as absent (inviting duplicate creates). Now a 2xx with an unusable payload throws a distinct, retryable `AutotaskResponseError`; void writes (update/delete) opt into a legitimately empty 2xx via `allowEmptyBody`; genuine `{items:[]}`, real 404s, and 204s are unaffected.
- **Missing-by-id GET returning `{item:null}` now reads as not-found.** Autotask answers a missing id with HTTP 200 `{item:null}` on several entities (not a 404); `get()`/`childGet()` previously returned the truthy wrapper. A new `unwrapEntity()` maps `{item:null}` to `null`, so not-found is reported correctly and read-after-write retry works.

### Added

- **Staff day + idempotent time logging for scheduled backfill** ([#42](https://github.com/GDSTechnology/autotask-mcp/issues/42), slice 3). Primitives for a scheduled personal assistant (ChatGPT, with calendar/meeting/email context elsewhere) to review the day and backfill Autotask safely.
  - `autotask_get_my_day` — the acting user's picture for a date (default today): tickets assigned to them, the time they've already logged that day, and their open tasks (fail-soft per section). Lets the assistant fill only the gaps.
  - `autotask_log_my_time` — logs time as the caller (via `currentUser`/gateway impersonation, role auto-filled) **idempotently**: before creating, it checks for an existing entry on the same day against the same ticket/task with the same `summaryNotes` and skips it, so a scheduled end-of-day run (and retries) never double-posts. The MCP owns safe reads/writes; the assistant owns the agenda-matching.
- **Trusted gateway impersonation for Teams handoff** ([#42](https://github.com/GDSTechnology/autotask-mcp/issues/42), slice 2). In gateway mode, the gateway (which has already authenticated the end user) may inject the acting Autotask identity via `X-Acting-Resource-Id` or `X-Acting-User-Email`. These are read **only** in gateway mode and only after the `/mcp` S2S gate has verified the request came from the gateway, then resolve the caller for `currentUser`/act-as — outranking the (spoofable) payload `requestingUserEmail`, but not an explicit in-call resource. Bound to the per-request, tenant-isolated handler only. Every impersonated action is attributable in the audit log (`impersonatedResourceId` / `impersonatedUserEmail`). A client cannot self-impersonate through `_meta`/`_context` — payload identity is never trusted for this.
- **Role resolution + `currentUser` ticket assignment** ([#42](https://github.com/GDSTechnology/autotask-mcp/issues/42), slice 1). Verified against the live role model (`Resources.defaultServiceDeskRoleID`, the `ResourceRoles` association, `Roles`).
  - `autotask_search_roles` (name/active) and `autotask_get_resource_roles` (a user's roles, name-enriched, default flagged) — discover a valid `roleID`.
  - `autotask_create_ticket` / `autotask_update_ticket` accept `currentUser: true` (assign to the caller); `assignedResourceRoleID` auto-fills from the resource's `defaultServiceDeskRoleID` when omitted, and time-entry `roleID` does the same. An explicitly-passed role always wins. This makes "assign this to me" one call (Autotask requires a role whenever a resource is assigned).
- **Revenue-first service automation — maintenance orchestrator + entitlement/coverage helpers** ([#63](https://github.com/GDSTechnology/autotask-mcp/issues/63)). Completes the MVP loop (build sheet §13/§14/§15).
  - `autotask_create_maintenance_ticket` — validates company → contract (active + not expired) → contract service → CI ownership/activity, checks `externalID` for an existing occurrence (no duplicate), then creates the ticket, links additional CIs, applies a checklist library, and reads it all back. `dryRun:true` validates and returns the plan **without writing** (for pilot/manual-approval runs, §21); `requireEntitlement:true` refuses an unentitled CI. The MCP does not own recurrence scheduling (§20).
  - `autotask_get_configuration_item_entitlement` — is a CI entitled to recurring service, and why. Normalized reasons (`ACTIVE_CI_ACTIVE_CONTRACT_SERVICE`, `ACTIVE_CI_NO_CONTRACT`, `CONTRACT_EXPIRED`, `CONTRACT_TERMINATED`, `CONTRACT_SERVICE_MISMATCH`, `SERVICE_NOT_MAPPED`, `CI_INACTIVE`) with supporting evidence. Contract active/expired is judged by the **tenant status label + endDate**, not a hardcoded numeric (verified live: this tenant uses `0=Inactive`/`1=Active`, where the old code comment assumed `3=Terminated`).
  - `autotask_search_configuration_item_coverage_gaps` — active CIs with no contract coverage, for Sales / Account review. Coverage is computed **in-memory**: Autotask's `eq null` / `noteExist` filters on CI foreign keys (`contractID`/`contractServiceID`) return zero rows even when null-valued records exist (verified live), so an API-side null filter would silently report no gaps.
- **Revenue-first service automation — checklist library application** ([#62](https://github.com/GDSTechnology/autotask-mcp/issues/62)). Standardized work instructions on maintenance tickets. Verified live against `ChecklistLibraries` ({id, name, description, isActive, entityType}) and `ChecklistLibraryChecklistItems` ({id, checklistLibraryID, itemName, isImportant, knowledgebaseArticleID, position}).
  - `autotask_search_checklist_libraries` (by `isActive`/`entityType`/name) and `autotask_get_checklist_library` (returns the library plus its items ordered by position).
  - `autotask_apply_checklist_library_to_ticket` — Autotask has no native "apply library" endpoint, so this expands the library's items into per-ticket checklist items (`itemName`/`isImportant`/`position`/`knowledgebaseArticleID` carried over). Each item is created independently; a failed item is reported per-item without aborting the rest.
- **Revenue-first service automation — ticket write + externalID idempotency** ([#61](https://github.com/GDSTechnology/autotask-mcp/issues/61)). The write side of the recurring-maintenance loop, with duplicate prevention. Field names verified live against the Tickets and `TicketAdditionalConfigurationItems` ({id, ticketID, configurationItemID}) schemas.
  - `autotask_create_ticket` gains contract/service linkage (`contractID`, `contractServiceID`, `contractServiceBundleID`), `externalID` (occurrence/idempotency key), `problemTicketId`, and a convenience `additionalConfigurationItemIDs[]` — when supplied, the ticket is created, each CI is linked, and the ticket + associations are read back (§12) and returned enriched (a CI link that fails is reported per-item; the created ticket id stays authoritative to avoid a duplicate on retry).
  - `autotask_find_ticket_by_external_id` + an `externalID` filter on `autotask_search_tickets` — the idempotency guard: look up an occurrence key before creating so a retried occurrence never creates a duplicate. `externalID` is now preserved through the ticket-search optimizer.
  - `autotask_search_ticket_configuration_items`, `autotask_add_ticket_configuration_item`, and `autotask_remove_ticket_configuration_item` (destructive — `confirm:true` gated) manage the additional-CI associations on a ticket.
- **Revenue-first service automation — CI + ContractService read foundation** ([#60](https://github.com/GDSTechnology/autotask-mcp/issues/60)). The read side of the recurring-maintenance loop: answer "which active assets does this contract/service cover, and is a CI still entitled?" All field names verified against the live schemas (ConfigurationItems = 107 fields, ContractServices = 10).
  - `autotask_search_configuration_items` gains the native entitlement filters — `companyLocationID`, `contractID`, `contractServiceID`, `contractServiceBundleID`, `serviceID`, `serviceBundleID`, `parentConfigurationItemID` — alongside the existing company/product/type/category/active filters. Returns full CI records, so every entitlement field is present in results.
  - `autotask_get_configuration_item` — single CI by id with all entitlement fields; optional `enrichReferences` resolves reference names (company, contract + status, service, contract service, product, parent CI) best-effort into an `_enriched` bag without failing the base read.
  - `autotask_search_contract_services` (by `contractID`/`serviceID`/`quoteItemID`) and `autotask_get_contract_service` — read the service entitlements on a contract, completing the ContractService surface (create/update already existed).
- **`autotask_get_complete_project_context`** — the Project Builder's read-only input (§10). Assembles a project's full context in one call: the project record, its nested phase/task hierarchy, the task dependency graph (batched `TaskPredecessors` by successor), the labor rollup, project notes and attachments, the owning company, and commercial linkage (contract + milestones, opportunity). Best-effort/fail-soft — each section is fetched independently and a section that errors is recorded under `errors` rather than failing the whole call. Configuration items are opt-in (`includeConfigurationItems`, can be large per company); `includeCommercial:false` skips the contract/opportunity fetches. Read-only; no writes.
- **Read-after-write verification for Project-Builder creates.** `autotask_create_project`/`_task`/`_phase` read the just-created entity back (bounded retry via `getWithRetry()` for Autotask's brief post-create read lag) and attach `verified` (+ `item` when visible) to the normalized result. The create's `itemId` stays authoritative, so a read that never resolves or errors yields `verified:false` — never a failed create (which would risk a duplicate on rerun). Opt-in per tool via a `verifyRead` flag in `CREATE_TOOL_META`.
- **TaskPredecessor CRUD completed** ([#46](https://github.com/GDSTechnology/autotask-mcp/issues/46) §7). Alongside the existing `list`/`add`/`remove_task_predecessor`, adds `autotask_get_task_predecessor`, `autotask_search_task_predecessors` (by `successorTaskID` and/or `predecessorTaskID`), and `autotask_update_task_predecessor`. Verified against the live entity: only `lagDays` is writable (both task references are readonly), so re-pointing a dependency is delete + recreate.
- **ContractMilestone CRUD** (§30). `autotask_get_contract_milestone`, `autotask_search_contract_milestones` (by `contractID`/`status`), `autotask_create_contract_milestone`, `autotask_update_contract_milestone`. Verified against the live entity: no delete (Autotask `canDelete=false`), `contractID` is required and readonly, `status` is a tenant picklist. Create/update are financial mutations requiring `confirm:true`; create is read-after-write verified.
