...
- **Revenue-first service automation — CI + ContractService read foundation** ([#60](https://github.com/GDSTechnology/autotask-mcp/issues/60)). The read side of the recurring-maintenance loop: answer "which active assets does this contract/service cover, and is a CI still entitled?" All field names verified against the live schemas (ConfigurationItems = 107 fields, ContractServices = 10).
  ...
  - `autotask_search_contract_services` (by `contractID`/`serviceID`/`quoteItemID`) and `autotask_get_contract_service` — read the service entitlements on a contract, completing the ContractService surface (create/update already existed). `autotask_create_project`/`_task`/`_phase` read the just-created entity back (bounded retry via `getWithRetry()` for Autotask's brief post-create read lag) and attach `verified` (+ `item` when visible) to the normalized result. The create's `itemId` stays authoritative, so a read that never resolves or errors yields `verified:false` — never a failed create (which would risk a duplicate on rerun). Opt-in per tool via a `verifyRead` flag in `CREATE_TOOL_META`.
